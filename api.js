...
export async function fetchPrevisao(city) {
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${city.latitude}` +
...
export function processarDadosPrevisao(data, city) {
  ...
  for (let d = 0; d < 10; d++) {
    ...
    cidadeAtualObj.forecast.push({
      ...
      // ESTA PARTE É CRUCIAL: Se faltar, dá o erro que você recebeu
      dadosHorarios: {
      horas: hourly.time.slice(i, i + 24).map(t => t.split("T")[1]),
      temperaturas: tempsDia,
      chuvas: chuvaDia,
      probabilidades: probDia,
      nebulosidade: cloudDia,
      trovoadas: codeDia.map(c => codigosTrovonada.includes(c))
    },
